...
#instead of changing food (food_combination_mutation), it changes the quantity of the food
def random_mutation(individual, elem_mute_rate):

    for i in range(len(individual)):

        if random() < elem_mute_rate:
            new_value = randint(0, 10)  # Randomly choose a new value between 0 and 10
            while new_value == individual[i]:
                new_value = randint(0, 10)
                
            individual[i] = new_value
    return individual

def geometric_mutation(individual, elem_mute_rate, scale_factor):
    mutated_individual = individual.copy()

    for i in range(len(mutated_individual)):
        if random() < elem_mute_rate:
            original_value = mutated_individual[i]
            mutated_value = original_value * random.randint(1 - scale_factor, 1 + scale_factor)
...
def insert_delete_mutation(individual):
    ...
    if operation == "insert":
        ...
        if zero_indices:
            index_to_add = random.choice(zero_indices)
        ...
        if nonzero_indices:
            index_to_delete = random.choice(nonzero_indices)
