...
def get_fitness(self):
    ...
    for i in range(len(min_nutrients)):
        if nutritional_values[i] < min_nutrients[i][1]:
            ...
            penalty += nutrient_penalty * 100  # Apply a penalty for each nutrient bellow requirement
...
pop = Population(size=100, optim="min", sol_size=len(data), valid_set=range(len(data)), replacement=True)
pop.evolve(pop=pop, generations=1000, select=fps, mutate=random_mutation, mutation_rate=0.5, crossover=multi_point_co, elite_size=2, no_improvement_threshold=100, plot= plot_c)
