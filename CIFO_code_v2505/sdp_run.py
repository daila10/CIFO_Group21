...
def random_initialization(quantity=201):
    return [randrange(quantity) for _ in range(58)] #expected quantity of food around 5kg per individual

def initialize_latin_hypercube(range_min=0, range_max=200):
...
def initialize_good(quantity=201):
   first_good = [randrange(quantity) for _ in range(35)] #first 35 food are 'good'
